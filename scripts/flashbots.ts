...
var tradesCycles = []
for(var i = 3; i < 6; i++) {
    var tradesCyclesSet = dfs.findArbs(tokenMap, i);
    for(var j = 0; j < tradesCyclesSet.length; j++) {
        tradesCycles.push(tradesCyclesSet[j])
    }
}
//tradesCycles = tradesCycles.flat(2)
const failedTrades = new Set()
const benchTrades = new Set()
setInterval(() => {
    benchTrades.clear()
}, 600000)
async function main() {
...
async function getTrades(arbContract) {
    ...
    await timeFunctionCall(common.updateReserves4Async, [tradesCycles, arbContract])
...
async function executeTrades(trades, arbContract, targetBlock, flashbotsProvider) {
    ...
    console.log("num of trades: " + trades.length)
    for(var i = 0; i < trades.length; i++) {
        if(!trades[i]) continue;
        if(failedTrades.has(JSON.stringify(trades[i].path))) continue;
        if(benchTrades.has(JSON.stringify(trades[i].path))) continue;
        ...
        if(EaEb[0] < EaEb[1] && optimalProfit > 0) {
            ...
            var baseFeeNextBlock = null
            try {
                baseFeeNextBlock = await timeFunctionCall(getBaseFeeNextBlock, [targetBlock])
            } catch(error) {
                continue
            }
            //const baseFeeCur = await getBaseFeeCurBlock(targetBlock)
            ...
            //flashbots simulates transaction using cur block base fee sometimes
            // const rawTransactionSimulate = {
            //     to: arbContract.address,
            //     gasPrice: baseFeeCur,//BigInt(Math.ceil(Number(BigInt(baseFeeNextBlock._hex)) * 1.127)),//BigInt(Math.floor(Number(baseFee) * 1.125)/* + BigInt(feeData.maxPriorityFeePerGas._hex)*/),
            //     gasLimit: 1700000,
            ...
            // const signedTransactionsForSim = await flashbotsProvider.signBundle([
            //     {
            //       signer: dependencies.wallet,
            //       transaction: rawTransactionSimulate
            //     }
            // ]);
            ...
            //if(common.getEthPriceUSD(feeData.gasPrice * 1000000 * 2) < trades[i].optimalProfitUSD) {
                ...
                if(simulation.error && simulation.error.message.includes("insufficient funds")) {
                    benchTrades.add(JSON.stringify(trades[i].path)); //cool off for 10 min
                    continue
                }
                if(simulation.results && (simulation.results[0].revert || simulation.results[0].error)) {
                    benchTrades.add(JSON.stringify(trades[i].path));
                    continue                    
                }
                if(simulation.error) {
                    benchTrades.add(JSON.stringify(trades[i].path));
...
async function getBaseFeeNextBlock(targetBlock) {
    ...
    const baseFeeNextBlock = dependencies.getBaseFeeInNextBlock(BigNumber.from(baseFee), currBlock.gasUsed, currBlock.gasLimit);
    const futureBlock = await dependencies.provider.getBlock(targetBlock)
    const maxBaseFeeInFutureBlock = await dependencies.getMaxBaseFeeInFutureBlock(currBlock.baseFeePerGas, 1)
    console.log("maxBaseFeeInFutureBlock: " + maxBaseFeeInFutureBlock)
    console.log("baseFeeNextBlock using flashbotsProvider: " + baseFeeNextBlock)
...
async function getBaseFeeCurBlock(targetBlock) {
    const currBlock = await dependencies.provider.getBlock(targetBlock - 1);
    const baseFee = BigInt(currBlock.baseFeePerGas._hex);
    return baseFee
}
