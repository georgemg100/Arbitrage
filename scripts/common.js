...
function getEaEb(tradeCycle) {
    const startToken = tradeCycle[0].token0.address;
    var Ea = (tradeCycle[0].reserve1 * tradeCycle[1].reserve1 * r)/(tradeCycle[1].reserve0 + tradeCycle[0].reserve1 * r);
    var Eb = (tradeCycle[0].reserve0 * tradeCycle[1].reserve0)/(tradeCycle[1].reserve0 +  tradeCycle[0].reserve1 * r);
    for(var i = 2; i < tradeCycle.length; i++) {
        Ea =(Ea * tradeCycle[i].reserve1 * r)/(tradeCycle[i].reserve0 + Ea * r);
        Eb = (Eb * tradeCycle[i].reserve0)/(tradeCycle[i].reserve0 + Ea * r);
    }
    return [Ea, Eb, startToken];
}

function getEaEb2(tradeCycle) {
    const startToken = tradeCycle[0].token0.address;
    var Ea = tradeCycle[0].reserve0;//(997 *tradeCycle[0].reserve1 * tradeCycle[1].reserve1)/(1000 * tradeCycle[1].reserve0 + tradeCycle[0].reserve1 * r);
    var Eb = tradeCycle[0].reserve1;//(tradeCycle[0].reserve0 * tradeCycle[1].reserve0)/(tradeCycle[1].reserve0 +  tradeCycle[0].reserve1 * r);
    for(var i = 1; i < tradeCycle.length; i++) {
        Eb =(Eb * tradeCycle[i].reserve1 * r)/(tradeCycle[i].reserve0 + Eb * r);
        Ea = (Ea * tradeCycle[i].reserve0)/(tradeCycle[i].reserve0 + Eb * r);
    }
    return [Ea, Eb, startToken];
}

function getEaEb3(tradeCycle) {
    const startToken = tradeCycle[0].token0.address;
    var Ea = BigInt(tradeCycle[0].reserve0);//(997 *tradeCycle[0].reserve1 * tradeCycle[1].reserve1)/(1000 * tradeCycle[1].reserve0 + tradeCycle[0].reserve1 * r);
    var Eb = BigInt(tradeCycle[0].reserve1);//(tradeCycle[0].reserve0 * tradeCycle[1].reserve0)/(tradeCycle[1].reserve0 +  tradeCycle[0].reserve1 * r);)
    for(var i = 1; i < tradeCycle.length; i++) {
        Ea = (Ea * BigInt(tradeCycle[i].reserve0) * BigInt(1000))/(BigInt(1000) * BigInt(tradeCycle[i].reserve0) + Eb * BigInt(997));
        Eb = (Eb * BigInt(tradeCycle[i].reserve1) * BigInt(997))/(BigInt(1000) * BigInt(tradeCycle[i].reserve0) + Eb * BigInt(997));
        //console.log("Ea: " + Ea);
        //console.log("Eb: " + Eb);
        if(Ea == 0 || Eb == 0) {
            return;
...
function getEaEb5(tradeCycle) {
    ...
    //const startToken = tradeCycle[0].token0.address;
    //var startToken = tradeCycle[0].token0.address != tradeCycle[1].token0.address && tradeCycle[0].token0.address != tradeCycle[1].token1.address ? tradeCycle[0].token0.address : tradeCycle[0].token1.address;
    const startTokenObj = startCoins.has(tradeCycle[0].token0.address) && firstPairTokens.has(tradeCycle[0].token0.address) && lastPairTokens.has(tradeCycle[0].token0.address)? tradeCycle[0].token0 : tradeCycle[0].token1;
    tradeCycle.decimalInput = startTokenObj.decimal;
